...
void PiezaTeletransporte::setMovimiento(int dir, bool estado)
{
	...
	if (dir == DIR_ARRIBA || dir == DIR_ABAJO)
	{
		if (estado)
		{
			ultimo_eje_y = (dir == DIR_ARRIBA) ? +1 : -1;
		}

		else
		{
			if (dir == DIR_ARRIBA && tecla_abajo)  ultimo_eje_y = -1;
			else if (dir == DIR_ABAJO && tecla_arriba) ultimo_eje_y = +1;
			else ultimo_eje_y = 0;
		}
	}
...
void PiezaTeletransporte::actualizarArena(double dt)
{
